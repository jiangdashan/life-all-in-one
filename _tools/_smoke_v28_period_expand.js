...
setTimeout(async()=>{
  ...
  /* ==================== v28 场景 1：cyclesExpanded 返回值结构 ==================== */
  const CE1=app.analyzePeriod(recs2).cyclesExpanded;
  R['v28_ce1_exists']=(CE1 && CE1.length===4)?'OK':'FAIL(len='+(CE1&&CE1.length)+')';
  R['v28_ce1_first_period']=(CE1 && CE1[0] && CE1[0].startDate===sC1 && CE1[0].endDate===eC1)?'OK':'FAIL';
  R['v28_ce1_first_days_len']=(CE1 && CE1[0] && CE1[0].days.length===daysBetween(sC1,eC1)+1)?'OK':'FAIL(days='+(CE1&&CE1[0]&&CE1[0].days.length)+' expected='+(daysBetween(sC1,eC1)+1)+')';
  R['v28_ce1_first_day_start']=(CE1 && CE1[0] && CE1[0].days[0] && CE1[0].days[0].source==='start')?'OK':'FAIL('+(CE1&&CE1[0]&&CE1[0].days[0]&&CE1[0].days[0].source)+')';
  R['v28_ce1_first_day_end']=(CE1 && CE1[0] && CE1[0].days[CE1[0].days.length-1] && CE1[0].days[CE1[0].days.length-1].source==='end')?'OK':'FAIL';
  R['v28_ce1_first_days_auto']=(CE1 && CE1[0] && CE1[0].days.filter(function(d){return d.source==='mid-auto';}).length===daysBetween(sC1,eC1)-1)?'OK':'FAIL';
  R['v28_ce1_last_open']=(CE1 && CE1[3] && CE1[3].hasEndMark===false)?'OK':'FAIL';

  /* ==================== v28 场景 2：每日手动打卡的 source = mid-manual ==================== */
  const sM=shiftDays(TODAY,-20); const m1=shiftDays(sM,1); const m2=shiftDays(sM,2); const eM=shiftDays(sM,4);
  const recsManual=[
    rec(sM,{isStartDay:'Y'}),
    rec(m1,{isPeriodDay:'Y',pad:2}),
    rec(m2,{isPeriodDay:'Y',pad:3}),
    rec(eM,{isEndDay:'Y'})
  ];
  const CE2=app.analyzePeriod(recsManual).cyclesExpanded;
  R['v28_ce2_len']=(CE2 && CE2.length===1)?'OK':'FAIL';
  R['v28_ce2_days_len']=(CE2 && CE2[0] && CE2[0].days.length===5)?'OK':'FAIL(days='+(CE2&&CE2[0]&&CE2[0].days.length)+')';
  const dayMap={}; (CE2[0].days||[]).forEach(function(d){dayMap[d.date]=d.source;});
  R['v28_ce2_start_src']=(dayMap[sM]==='start')?'OK':'FAIL('+dayMap[sM]+')';
  R['v28_ce2_end_src']=(dayMap[eM]==='end')?'OK':'FAIL('+dayMap[eM]+')';
  R['v28_ce2_manual_m1']=(dayMap[m1]==='mid-manual')?'OK':'FAIL('+dayMap[m1]+')';
  R['v28_ce2_manual_m2']=(dayMap[m2]==='mid-manual')?'OK':'FAIL('+dayMap[m2]+')';
  R['v28_ce2_mid_mid']=(dayMap[shiftDays(sM,3)]==='mid-auto')?'OK':'FAIL('+dayMap[shiftDays(sM,3)]+')';

  /* ==================== v28 场景 3：极端 case - 用户每天单独打卡，没用首末日 ==================== */
  const sD1=shiftDays(TODAY,-90); const d1=shiftDays(sD1,1); const d2=shiftDays(sD1,2); const d3=shiftDays(sD1,3); const eD1=shiftDays(sD1,4);
  const recsDayByDay=[
    rec(sD1,{isStartDay:'Y',isPeriodDay:'Y'}),
    rec(d1,{isPeriodDay:'Y',pad:2}),
    rec(d2,{isPeriodDay:'Y',pad:2}),
    rec(d3,{isPeriodDay:'Y',pad:2}),
    rec(eD1,{isPeriodDay:'Y',isEndDay:'Y',pad:1})
  ];
  const CE3=app.analyzePeriod(recsDayByDay).cyclesExpanded;
  R['v28_ce3_len']=(CE3 && CE3.length===1)?'OK':'FAIL';
  R['v28_ce3_days_len']=(CE3 && CE3[0] && CE3[0].days.length===5)?'OK':'FAIL';
  const dayMap3={}; (CE3[0].days||[]).forEach(function(d){dayMap3[d.date]=d.source;});
  R['v28_ce3_all_manual']=(dayMap3[sD1]==='start' && dayMap3[eD1]==='end' && dayMap3[d1]==='mid-manual' && dayMap3[d2]==='mid-manual' && dayMap3[d3]==='mid-manual')?'OK':'FAIL('+JSON.stringify(dayMap3)+')';
  R['v28_ce3_no_auto_days']=(CE3[0].days.filter(function(d){return d.source==='mid-auto';}).length===0)?'OK':'FAIL('+CE3[0].days.filter(function(d){return d.source==='mid-auto';}).length+')';

  /* ==================== v28 场景 4（核心）：只标首末 → state.records 不增加自动记录 ==================== */
  const sOnly=shiftDays(TODAY,-50); const eOnly=shiftDays(sOnly,3); /* 用户只标首日+3天后的末日 */
  app.state.records = app.state.records.filter(r => r.type !== 'period');
  const recordsBefore = app.state.records.length;
  const retA = app.addRecord('period', sOnly, {isStartDay:'Y', isPeriodDay:'Y', isEndDay:'', discharge:'', symptoms:[], moodScore:5, moodLabel:'', bleeding:3, pad:0, night:0, note:''});
  const retB = app.addRecord('period', eOnly, {isStartDay:'', isPeriodDay:'Y', isEndDay:'Y', discharge:'', symptoms:[], moodScore:5, moodLabel:'', bleeding:1, pad:0, night:0, note:''});
  const recordsAfter = app.state.records.length;
  /* 关键：不会自动给中间日创建新 record */
  R['v28_only2adds_2records']=(recordsAfter===recordsBefore+2)?'OK':'FAIL(diff='+(recordsAfter-recordsBefore)+')';
  R['v28_only2adds_no_middle_records']=((recordsAfter-recordsBefore)<=2)?'OK':'FAIL(diff='+(recordsAfter-recordsBefore)+')';
  /* analyzePeriod 看到 2 条记录，但展开 5 天 */
  const CE4 = app.analyzePeriod(app.state.records).cyclesExpanded;
  R['v28_ce4_len']=(CE4 && CE4.length===1)?'OK':'FAIL';
  R['v28_ce4_days_len']=(CE4 && CE4[0] && CE4[0].days.length===4)?'OK':'FAIL(days='+(CE4&&CE4[0]&&CE4[0].days.length)+' expected=4 - shiftDays(sOnly,3)是sOnly+3共4天)';
  const dayMap4={}; (CE4[0].days||[]).forEach(function(d){dayMap4[d.date]=d.source;});
  R['v28_ce4_m1_auto']=(dayMap4[shiftDays(sOnly,1)]==='mid-auto')?'OK':'FAIL';
  R['v28_ce4_m2_auto']=(dayMap4[shiftDays(sOnly,2)]==='mid-auto')?'OK':'FAIL';
  R['v28_ce4_start']=(dayMap4[sOnly]==='start')?'OK':'FAIL';
  R['v28_ce4_end']=(dayMap4[eOnly]==='end')?'OK':'FAIL';

  /* ==================== v28 场景 5：脏数据防御 —— 历史 3 条同日重复标首末（v27 教训）==================== */
  const dirtyDay = shiftDays(TODAY,-15);
  const dirtyPeriodRecs = [
    rec(dirtyDay, {isStartDay:'Y', isPeriodDay:'Y', isEndDay:'', pad:1}),
    rec(dirtyDay, {isStartDay:'Y', isPeriodDay:'', isEndDay:'', pad:1}),  /* 同日重复，不应新增 */
    rec(dirtyDay, {isStartDay:'', isPeriodDay:'Y', isEndDay:'', pad:1}),
  ];
  app.state.records = app.state.records.filter(r => r.type !== 'period');
  const dirtyBefore = app.state.records.length;
  app.addRecord('period', dirtyDay, {isStartDay:'Y',isPeriodDay:'Y',isEndDay:'',pad:1});
  app.addRecord('period', dirtyDay, {isStartDay:'Y',isPeriodDay:'',isEndDay:'',pad:1});
  app.addRecord('period', dirtyDay, {isStartDay:'',isPeriodDay:'Y',isEndDay:'',pad:1});
  const dirtyAfter = app.state.records.length;
  R['v28_dirty_no_dup_3adds_1record']=(dirtyAfter===dirtyBefore+1)?'OK':'FAIL(diff='+(dirtyAfter-dirtyBefore)+')';
  const CE5 = app.analyzePeriod(app.state.records).cyclesExpanded;
  R['v28_dirty_ce_len']=(CE5 && CE5.length===1)?'OK':'FAIL';
  R['v28_dirty_ce_days_len']=(CE5 && CE5[0] && CE5[0].days.length===16)?'OK':'FAIL(days='+(CE5&&CE5[0]&&CE5[0].days.length)+' - 当前未闭合周期没末日展开到today是正确行为)';

  /* ==================== v28 场景 6：DOM 渲染包含展开按钮，每日详情 hidden ==================== */
  app.state.records = recs2;
  app.renderPeriod();
  await new Promise(r=>setTimeout(r,50));
  const histHtml = doc.getElementById('periodHistory').innerHTML;
  R['v28_dom_render_expand_btn']=(histHtml.indexOf('data-action=\"toggle-cycle\"')>=0)?'OK':'FAIL';
  const panel0 = doc.getElementById('cycle-days-0');
  R['v28_dom_panel_initial_hidden']=(panel0 && panel0.hidden===true)?'OK':'FAIL';
  R['v28_dom_days_count_in_dom']=(histHtml.match(/class=\"period-day-row /g)||[]).length >= 5 ? 'OK' : 'FAIL('+(histHtml.match(/class=\"period-day-row /g)||[]).length+' rows)';
  /* 验证每条 period-day-row 的 source class 正确 */
  R['v28_dom_has_start_class']=(histHtml.indexOf('period-day-row start')>=0)?'OK':'FAIL';
  R['v28_dom_has_end_class']=(histHtml.indexOf('period-day-row end')>=0)?'OK':'FAIL';
  R['v28_dom_has_midauto_class']=(histHtml.indexOf('period-day-row mid-auto')>=0)?'OK':'FAIL';

  /* ==================== v28 场景 7：点击 toggle-cycle → 取消 hidden + aria-expanded 切换 ==================== */
  if(panel0){
    try{
      panel0.dispatchEvent(new w.Event('click',{bubbles:true}));
      const btn = doc.querySelector('[data-action=\"toggle-cycle\"][data-cycle-idx=\"0\"]');
      btn.dispatchEvent(new w.Event('click',{bubbles:true,cancelable:true}));
      await new Promise(r=>setTimeout(r,20));
      R['v28_toggle_unhidden']=(panel0.hidden===false)?'OK':'FAIL(hidden='+panel0.hidden+')';
      R['v28_toggle_aria']=(btn.getAttribute('aria-expanded')==='true')?'OK':'FAIL';
      R['v28_toggle_class']=(btn.classList.contains('expanded'))?'OK':'FAIL';
      /* 再点一次 → 折叠回去 */
      btn.dispatchEvent(new w.Event('click',{bubbles:true,cancelable:true}));
      await new Promise(r=>setTimeout(r,20));
      R['v28_toggle_recollapse']=(panel0.hidden===true)?'OK':'FAIL(hidden='+panel0.hidden+')';
    }catch(e){ R['v28_toggle_unhidden']='ERR('+e.message+')'; R['v28_toggle_aria']='SKIP'; R['v28_toggle_class']='SKIP'; R['v28_toggle_recollapse']='SKIP'; }
  } else {
    R['v28_toggle_unhidden']='SKIP(no panel)'; R['v28_toggle_aria']='SKIP'; R['v28_toggle_class']='SKIP'; R['v28_toggle_recollapse']='SKIP';
  }

  /* ==================== v28 场景 8：核心原则 —— state.records 永远是用户的真实输入，绝不自动增长 ==================== */
  app.state.records = app.state.records.filter(r => r.type !== 'period');
  const realInput = app.state.records.length;
  app.addRecord('period', sOnly, {isStartDay:'Y',isPeriodDay:'Y'});
  app.renderPeriod();
  const afterRender = app.state.records.length;
  R['v28_principle_render_no_record_growth']=(afterRender===realInput+1)?'OK':'FAIL(diff='+(afterRender-realInput)+')';
  R['v28_principle_no_auto_create_in_render']=(afterRender<=realInput+1)?'OK':'FAIL(diff='+(afterRender-realInput)+')';

  /* ==================== v28 场景 9：回归 v26 情绪统计（手动日才计分，自动日不计分） ==================== */
  const eRecs=[
    rec(shiftDays(TODAY,-90),{isStartDay:'Y',moodScore:3}),
    rec(shiftDays(TODAY,-89),{isPeriodDay:'Y',moodScore:4}),  /* 手动日记，仅计分 */
    rec(shiftDays(TODAY,-86),{isEndDay:'Y',moodScore:5})
  ];
  /* cyclesExpanded 每条 day 应该带 record，且 record 的 moodScore 正确 */
  const CE_emotion = app.analyzePeriod(eRecs).cyclesExpanded[0];
  const manualDays = (CE_emotion.days||[]).filter(function(d){return d.source==='mid-manual';});
  R['v28_emotion_mid_manual_has_score']=(manualDays.length===1 && manualDays[0].record && manualDays[0].record.data.moodScore===4)?'OK':'FAIL';
  const startDay = (CE_emotion.days||[]).find(function(d){return d.source==='start';});
  R['v28_emotion_start_has_record']=(startDay && startDay.record && startDay.record.data.moodScore===3)?'OK':'FAIL';
