...
4. **移除默认邀请码明文**：旧码 RJ-VKD5-YWLY 从全项目源码消失，换新随机码 RJ-U6M9-VBCV（AUTH_INVITE_DEFAULT 与 AUTH_INVITE_HASH 摘要 bf0a8115... 同步替换，SHA256 校验一致）。已设自定义码的用户走 localStorage 分支不受影响。注意：换默认码后，靠旧默认码+密码建的独立空间换新设备用旧码验证会失败。
